...
public:
	CSuperLeaf(float x = 0, float y = 0, DirectionXAxisType nx = DirectionXAxisType::Left, float vx = 0, float vy = SUPER_LEAF_FALL_SPEED, float ax = 0, float ay = 0, float swaySpeed = SUPER_LEAF_SWAY_SPEED, float swayAmplitude = SUPER_LEAF_SWAY_AMPLITUDE): CItem(x, y, vx, vy, ax, ay, nx), startX(x), swaySpeed(swaySpeed), swayAmplitude(swayAmplitude), timelapse(0) {}
