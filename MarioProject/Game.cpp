...
void CGame::_ParseSection_SETTINGS(string line) {
	...
	if (tokens[0] == "start") {
		nextScene = atoi(tokens[1].c_str());
	} else if (tokens[0] == "width") {
		backBufferWidth = atoi(tokens[1].c_str());
	} else if (tokens[0] == "height") {
		backBufferHeight = atoi(tokens[1].c_str());
	} else {
...
void CGame::_ParseSection_TEXTURES(string line) {
	...
	// [TEXTURES]
	// id	file_path
	// 1	textures\\mario.png
	if (tokens.size() < 2) return;
	int texID = atoi(tokens[0].c_str());

	wstring path = ToWSTR(tokens[1]);
	//LPTEXTURE tex = LoadTexture(path.c_str());
...
void CGame::LoadDataSection(int section, string line)
{
	switch (section)
	{
...
int CGame::GetSection(string line) {
	if (line == "[SETTINGS]") return GAME_FILE_SECTION_SETTINGS;
	if (line == "[TEXTURES]") return GAME_FILE_SECTION_TEXTURES;
	if (line == "[SCENES]") return GAME_FILE_SECTION_SCENES;
	if (line[0] == '[') {
		DebugOut(L"[ERROR] Unknown section: %s\n", ToLPCWSTR(line));
		return GAME_FILE_SECTION_UNKNOWN;
	};
	return -1000;
}
...
void CGame::Load(LPCWSTR gameFile) {
	...
	while (f.getline(str, MAX_GAME_LINE))
	{
		string line(str);
		if (line[0] == '#') continue;
		if (GetSection(line) >= -1) {
			DebugOut(L"[INFO] Loading section : %d\n", GetSection(line));
			section = GetSection(line);
			continue;
		}
		LoadDataSection(section, line);
	}
	f.close();
